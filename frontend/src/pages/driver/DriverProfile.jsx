...
export default function DriverProfile() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [profileRes, ratingRes] = await Promise.all([
        ...
        API.get(`/drivers/rating-stats/${driverId}`).catch(() => ({
          data: null,
        })),
      ]);
  ...
  const toggleStatus = async () => {
    ...
    const isOnline =
      profile.status === "available" || profile.status === "free";
    const newStatus = isOnline ? "inactive" : "available";
    const toastId = toast.loading("Đang cập nhật...");
    try {
      await API.patch(`/drivers/toggle-status/${driverId}`, {
        status: newStatus,
      });
  ...
  const handleChangePassword = async (e) => {
    ...
    if (newPassword.length < 6)
      return toast.error("Mật khẩu mới ít nhất 6 ký tự");
      ...
      <div className="relative bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="px-6 pb-6 relative flex flex-col md:flex-row items-center md:items-end -mt-12 gap-4 md:gap-6">
          ...
          <div className="flex items-center gap-3 mt-2 md:mt-0">
            ...
            <button
              onClick={fetchData}
              className="p-2.5 rounded-xl bg-gray-50 hover:bg-white text-gray-600 border border-gray-200 hover:shadow transition-all"
              title="Làm mới"
            >
              <RefreshCw
                size={20}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              {!isEditing ? (
                <button
                  ...
                  className="text-blue-600 hover:bg-blue-50 p-1.5 rounded-lg transition flex items-center gap-1 text-sm"
                >
                  <Edit size={16} /> Sửa
                </button>
              ) : (
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-gray-400 hover:bg-gray-50 p-1.5 rounded-lg transition"
                  >
                    <X size={16} />
                  </button>
                </div>
            ...
            {isEditing ? (
              <div className="space-y-4">
                <div>
                  <label className="text-xs text-gray-400 font-medium uppercase mb-1 block">
                    Họ tên
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm transition"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-400 font-medium uppercase mb-1 block">
                ...
                <InfoRow
                  ...
                  label="Phương tiện"
                  value={
                    profile.vehicle_type || profile.type || "Chưa cập nhật"
                  }
                />
                <InfoRow
                  icon={MapPin}
                  label="Biển số xe"
                  value={profile.plate_no || "Chưa gán"}
                />
              </div>
            ) : (
              <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 hover:text-[#113e48] transition-colors flex items-center justify-center gap-2"
            >
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatBox
              label="Tổng đơn"
              value={ratingStats?.total_orders || 0}
              icon={PackageCheck}
              color="blue"
            />
            <StatBox
              label="Hoàn tất"
              value={ratingStats?.completed_orders || 0}
              icon={Award}
              color="green"
            />
            <StatBox
          ...
          {ratingStats?.recent_feedbacks?.length > 0 && (
            ...
            >
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                <MessageSquare size={18} className="text-[#113e48]" /> Đánh giá
                gần đây
              </h3>
              <div className="space-y-4">
                        ...
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_, j) => (
                            <Star
                              key={j}
                              size={12}
                              fill={j < fb.rating ? "currentColor" : "none"}
                              className={
                                j < fb.rating
                                  ? "text-yellow-400"
                                  : "text-gray-200"
            ...
            >
              ...
              <form onSubmit={handleChangePassword} className="p-6 space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-[#113e48] to-[#2a6f7d] text-white rounded-xl font-bold hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {savingPassword ? (
...
const StatBox = ({ label, value, icon: Icon, color, subValue }) => {
  ...
  return (
    ...
    >
      <div className={`p-3 rounded-full ${c[color]} mb-1`}>
        <Icon size={24} />
      </div>
  ...
  <div>
    ...
    <div className="relative">
      <input
        ...
        className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm pr-10 transition"
        placeholder={label}
      />
      <button
        type="button"
